...
const Booking = () => {
  ...
  // Формируем дату строго из локальных компонентов (год/месяц/день) — никогда через
  // toISOString(), которая переводит в UTC и может "съехать" на день из-за часового пояса.
  ...
  const viewMonthBase = today.getMonth();
  const viewDate = new Date(viewYear, viewMonthBase + monthOffset, 1);
  const calYear = viewDate.getFullYear();
  const calMonth = viewDate.getMonth();
  const daysInMonth = new Date(calYear, calMonth + 1, 0).getDate();

  // Пн=0 ... Вс=6 для верстки календарной сетки
  ...
  const calendarDays = Array.from({ length: daysInMonth }, (_, i) => {
    ...
    return {
      value: toDateStr(calYear, calMonth, day),
      day,
  ...
  const formatDateLabel = (dateStr: string) => {
    ...
    return `${pad2(d)}.${pad2(m)} ${weekdaysFull[dateObj.getDay()]}`;
  };
  ...
  // Дату разбираем вручную на компоненты и строим локальный Date — это исключает
  // любые сдвиги из-за часового пояса, в отличие от new Date("YYYY-MM-DD").
  const isWeekendDate = (dateStr: string) => {
    if (!dateStr) return false;
    const [y, m, d] = dateStr.split('-').map(Number);
    const day = new Date(y, m - 1, d).getDay(); // 0 = вс, 6 = сб
    return day === 0 || day === 6;
  };
  ...
  const modes = [
    { value: 'standard', label: 'Стандартный (14+)', price: 0, description: 'Базовый сюжет с полным погружением в атмосферу Зоны' },
    { value: 'light', label: 'Лайт (12+)', price: 0, description: 'Облегчённая версия с меньшим уровнем страха' },
    { value: 'kids', label: 'Детский (10+)', price: 0, description: 'Изменённый сюжет с минимальным количеством хоррор-элементов' },
  ];
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center justify-between mb-3">
                  <button
                    ...
                    className="w-9 h-9 flex items-center justify-center rounded-lg bg-zinc-800 text-orange-400 disabled:opacity-30 disabled:cursor-not-allowed hover:bg-zinc-700"
                ...
                <div className="grid grid-cols-7 gap-1">
                  ...
                  {calendarDays.map((date) => {
                    const isWeekendCell = date.weekdayIdx === 0 || date.weekdayIdx === 6;
                    return (
                      <button
                        key={date.value}
                        onClick={() => !date.isPast && setSelectedDate(date.value)}
                        disabled={date.isPast}
                        className={`aspect-square rounded-lg text-center text-sm transition-all duration-200 flex flex-col items-center justify-center ${
                        ...
                        } ${date.isToday && selectedDate !== date.value ? 'ring-1 ring-orange-500/60' : ''}`}
                      >
                        {date.day}
                      </button>
                    );
                ...
                <p className="text-xs text-gray-500 mt-2">Суббота и воскресенье выделены оранжевым — цены на них отличаются.</p>
              </div>
                ...
                <div className="grid sm:grid-cols-2 gap-4 mb-6">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <textarea
                      ...
                      placeholder="Например: день рождения, аллергии, пожелания по музыке..."
                      rows={3}
                      className="w-full bg-zinc-800 text-gray-300 border border-orange-500/30 rounded-lg px-4 py-3 focus:outline-none focus:border-orange-500 resize-none"
                    />
                  </div>
                </div>
              )}
          ...
          <div className="stalker-card p-6 max-w-md w-full">
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="border-t border-orange-500/30 pt-3 flex justify-between">
                ...
                <span className="text-orange-500 font-bold text-xl font-data">{getTotalPrice()} р.</span>
              </div>
            </div>
            <div className="space-y-3">
              <button
